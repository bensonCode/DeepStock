...
index_data_org = pd.read_csv('./data/tx/2017tx.csv', parse_dates=[0])
# 保留这几个需要的字段：'date', 'high', 'low', 'close', 'change'
# index_data = index_data_org[['date', 'high', 'low', 'close', 'fluctuation']]
data = [index_data_org[0], index_data_org['high'], index_data_org['low'], index_data_org['close'], index_data_org['fluctuation']]

index_data = pd.concat(data, axis=1)
...
index_data['最近N1个交易日的最高点'] = index_data['high'].rolling(window=N1, center=False).max()
# 对于上市不足N1天的数据，取上市至今的最高价
# index_data['最近N1个交易日的最高点'].fillna(value=pd.expanding_max(index_data['high']), inplace=True)
index_data['最近N1个交易日的最高点'].fillna(value=index_data['high'].expanding(min_periods=1).max())

# 通过相似的方法计算最近N2个交易日的最低价
# index_data['最近N2个交易日的最低点'] = pd.rolling_min(index_data['low'], N1)
index_data['最近N2个交易日的最低点'] = index_data['low'].rolling(window=N1, center=False).min()
# index_data['最近N2个交易日的最低点'].fillna(value=pd.expanding_min(index_data['low']), inplace=True)
index_data['最近N2个交易日的最低点'].fillna(value=index_data['low'].expanding(min_periods=1).min())
...
index_data[['date', 'high', 'low', 'close', 'fluctuation', '最近N1个交易日的最高点',
            '最近N2个交易日的最低点', '当天的仓位', '资金指数']].to_csv('turtle.csv', index=False, encoding='utf-8')

# ==========计算每年指数的收益以及海龟交易法则的收益
index_data['海龟法则每日涨跌幅'] = index_data['fluctuation'] * index_data['当天的仓位']
# year_rtn = index_data.set_index('date')[['fluctuation', '海龟法则每日涨跌幅']].resample(how=lambda x: (x + 1.0).prod() - 1.0) * 100
# print(year_rtn)
...
plt.plot(index_data['date'], color='red', label='Prediction')
plt.plot(index_data['资金指数'], color='blue', label='Actual')
plt.legend(loc='best')
plt.show()
